"""
Experiment 5 — Spatial + FFT + Temporal Transformer Model.

Architecture:
- Spatial Branch: ImageNet ResNet-50 (Layers 1-2 Frozen, Layers 3-4 Fine-Tuned @ LR 1e-5) -> 256-D per frame
- Frequency Branch: 2D FFT Module -> Frequency CNN -> 256-D per frame
- Frame Feature Fusion: Concatenate (512-D) -> Linear Projection -> 256-D per frame
- Temporal Transformer Encoder: 2 Layers, 4 Heads, d_model=256, dim_feedforward=512, dropout=0.1
- Temporal Aggregation: Mean pooling across unpadded frame sequence
...
class SpatialFFTTemporalDetector(nn.Module):
    """
    Spatial (Fine-Tuned ResNet-50) + FFT Frequency CNN + Multi-Head Temporal Transformer
    """
    def __init__(self, feature_dim: int = 256, num_heads: int = 4, num_layers: int = 2, dropout: float = 0.1):
        ...
        # Frame Feature Projection: 512-D (Spatial 256 + Freq 256) -> 256-D
        self.frame_fusion_proj = nn.Sequential(
    ...
    def forward(self, face_frames: torch.Tensor, padding_mask: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        flat_spatial = torch.cat(all_spatial, dim=0).view(B, N, -1) # (B, N, 256)
        flat_freq = torch.cat(all_freq, dim=0).view(B, N, -1)       # (B, N, 256)

        # Concatenate per-frame spatial + frequency features -> (B, N, 512)
        frame_features_512 = torch.cat([flat_spatial, flat_freq], dim=-1)
        
        # Project per-frame to 256-D
        frame_seq = self.frame_fusion_proj(frame_features_512) # (B, N, 256)
        ...
        if padding_mask is not None:
            # Transformer expects src_key_padding_mask as True for padded elements
...
def train_spatial_fft_temporal(
    epochs: int = 10,
    batch_size: int = 4,
    ...
    output_checkpoint: str = "model/checkpoints/spatial_fft_temporal_best.pt"
):
    project_root = Path(__file__).resolve().parents[2]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n--- STARTING EXPERIMENT 5: SPATIAL + FFT + TEMPORAL TRANSFORMER ON {device} ---")
    ...
    model = SpatialFFTTemporalDetector(feature_dim=256, num_heads=4, num_layers=2, dropout=0.1).to(device)
    ...
    transformer_params = list(model.frame_fusion_proj.parameters()) + [model.pos_encoder] + list(model.transformer_encoder.parameters())
    ...
    print("=" * 95)
    print(f"Experiment 5 (Spatial + FFT + Temporal) Complete! Best Validation ROC-AUC: {best_val_auc * 100:.2f}%")
    print(f"Saved best checkpoint to: {out_path}\n")


if __name__ == "__main__":
    train_spatial_fft_temporal(epochs=10, batch_size=4, spatial_lr=1e-5, fft_lr=3e-4, transformer_lr=1e-4, classifier_lr=1e-4)
