...
Stage B — Fine-Tuning Backbone (Unfrozen Later Layers):
- ResNet-50 with torchvision ImageNet weights (ResNet50_Weights.DEFAULT)
- Layers 1 & 2: FROZEN
- Layers 3 & 4 + FC Projection: TRAINABLE
- Differential Learning Rates: Backbone LR = 1e-5, Classifier LR = 1e-4
...
class PretrainedSpatialDetectorFineTune(nn.Module):
    """
    ImageNet Pretrained ResNet-50 Fine-Tuning Model:
    - Layer1, Layer2: Frozen
    - Layer3, Layer4: Unfrozen (Trainable @ LR 1e-5)
    - Head (Projection + Classifier): Trainable @ LR 1e-4
    """
    def __init__(self, feature_dim: int = 256, dropout: float = 0.1):
        ...
        # Separate modules for layer-wise unfreezing
    ...
    def forward(self, face_frames: torch.Tensor, padding_mask: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(0, B * N, chunk_size):
            ...
            cnn_out = self.extract_features(chunk) # (C, 2048)
...
def train_pretrained_stage_b(
    ...
    output_checkpoint: str = "model/checkpoints/spatial_pretrained_stage_b_best.pt"
):
    project_root = Path(__file__).resolve().parents[2]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n--- STARTING EXPERIMENT 3 STAGE B: PRETRAINED RESNET-50 FINE-TUNING ON {device} ---")
    ...
    model = PretrainedSpatialDetectorFineTune(feature_dim=256, dropout=0.1).to(device)
    ...
    print("=" * 90)
    print(f"Pretrained ResNet-50 Stage B Complete! Best Validation ROC-AUC: {best_val_auc * 100:.2f}%")
    print(f"Saved best checkpoint to: {out_path}\n")


if __name__ == "__main__":
    train_pretrained_stage_b(epochs=10, batch_size=4, backbone_lr=1e-5, classifier_lr=1e-4)
