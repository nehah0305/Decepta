"""
Standalone Experiment 1: Spatial CNN Alone Training & Evaluation.

Architecture:
FF++ Face Frames -> Spatial CNN (ResNet-50 256-D) -> Frame Average Pooling -> Linear Classifier (256 -> 1)

Features:
- Pure Spatial Baseline (No FFT, No Transformer, No Audio, No Sync, No Fusion)
- 50:50 Balanced Sampling via WeightedRandomSampler
- BCEWithLogitsLoss
- Backbone LR = 2e-5, Classifier LR = 1e-4
...
class PureSpatialDetector(nn.Module):
    """
    Pure Spatial CNN Model:
    Face Frames -> SpatialCNN -> Frame Average Pooling -> Linear Classifier
    """
    ...
    def forward(self, face_frames: torch.Tensor, padding_mask: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Process frames in chunks of 32
        chunk_size = 32
        all_feats = []
        for i in range(0, B * N, chunk_size):
            chunk = flat_frames[i:i + chunk_size]
            feat = self.spatial_cnn(chunk) # (C, 256)
...
def compute_embedding_separability(model: nn.Module, val_loader: DataLoader, device: torch.device) -> Tuple[float, float]:
    """Extracts 25 Real and 25 Fake embeddings and computes Centroid L2 Distance and Cosine Similarity."""
    model.eval()
    real_embeds = []
    fake_embeds = []
...
def train_spatial_experiment(
    epochs: int = 10,
    batch_size: int = 4,
    backbone_lr: float = 3e-4,
    classifier_lr: float = 3e-4,
    train_manifest: str = "Datasets/metadata/train.csv",
    val_manifest: str = "Datasets/metadata/validation.csv",
    data_root: str = "Datasets/raw/faceforensicspp",
    output_checkpoint: str = "model/checkpoints/spatial_cnn_best.pt"
):
    project_root = Path(__file__).resolve().parents[2]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n--- STARTING STANDALONE SPATIAL CNN EXPERIMENT ON {device} ---")
    ...
    # WeightedRandomSampler for 50:50 Real vs Fake training
    ...
    model = PureSpatialDetector(feature_dim=256, dropout=0.1).to(device)

    # Optimizer with differential learning rates: backbone 2e-5, classifier 1e-4
    optimizer = torch.optim.AdamW([
        {"params": model.spatial_cnn.parameters(), "lr": backbone_lr},
    ...
    for epoch in range(1, epochs + 1):
        ...
        l2_dist, cos_sim = compute_embedding_separability(model, val_loader, device)
        ...
        # Checkpoint selection on best validation ROC-AUC
    ...
    print("=" * 105)
    print(f"Standalone Spatial CNN Experiment Complete! Best Validation ROC-AUC: {best_val_auc * 100:.2f}%")
    print(f"Saved best checkpoint to: {out_path}\n")


if __name__ == "__main__":
    train_spatial_experiment(epochs=10, batch_size=4, backbone_lr=3e-4, classifier_lr=3e-4)
